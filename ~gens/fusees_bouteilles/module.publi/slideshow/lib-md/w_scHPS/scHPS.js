// noinspection ExceptionCaughtLocallyJS,JSCommentMatchesSignature
...
window.scHPS = {
	...
	fStrings : [
		...
		/*04*/ "Fermer","Fermer le zoom (Échap)",
		/*06*/ "","",
		/*08*/ "Mode HTML","Basculer en mode HTML simple (H)",
		/*10*/ "Êtes-vous sûr de vouloir basculer vers une version HTML simple de ce diaporama ?","Effets",
	...
	init : function() {
		try{
			...
			if (window.location.search.indexOf("mode=html")<0) this.fDisabled = false;
			this.fStore = new scHPS.LocalStore();
		} catch(e){console.error("ERROR scHPS.init : " + e);}
	},
	/** scHPS.swichToSsStyles : Switch to slide-show CSS files.
	 * @param pDoc (optional) : document to switch. */
	swichToSsStyles : function(pDoc) {
		const vDoc = (typeof pDoc == "undefined" ? document : pDoc);
		if (!vDoc.fCssSwiched){
			let vCss = null;
			for(let i = 0; (vCss = vDoc.getElementsByTagName("link")[i]); i++) {
				if (vCss.getAttribute("rel").indexOf("style") !== -1){
					const vTitle = vCss.getAttribute("title") || "";
	...
	xAddBtn : function(pParent, pMgr, pFunc, pClassName, pCapt, pTitle, pNxtSib) {
		const vBtn = pParent.ownerDocument.createElement(scHPS.fButtonElement);
		vBtn.className = pClassName;
		vBtn.fName = pClassName;
		if (scHPS.fButtonElement==="a"){
			vBtn.href = "#";
			vBtn.target = "_self";
		}
		vBtn.fMgr = pMgr;
	...
	xSerialiseObjJs : function(pObj){
		...
		if(pObj) for (let vKey in pObj){
			const vLbl = (pObj instanceof Array) ? "" : "\'" + vKey + "\':";
			const vObj = pObj[vKey];
			if(vObj != null) {
				vBuf+= (vBuf!=="" ? "," : "") + vLbl;
...
scHPS.importElt = function(pEltSrc, pDocDst, pParentDst, pChildDst) {
	const vRes = pDocDst.importNode(pEltSrc, false);
...
scHPS.FadeEltTask = function(pElt,pDir,pMgr,pInstant){
	this.fRate = [];
...
scHPS.PresMgr.prototype = {
	...
	onLoad : function() {
		...
		try{
			...
			if (this.fIsMaster){
				...
				window.addEventListener("mousemove", function(){scHPS.sMouseMgr(vPresMgr)},true);
				window.addEventListener("mousedown", function(){scHPS.sMouseMgr(vPresMgr)},true);
			...
			if (this.fToolsPath){
				...
				if (!this.fEnableCssEffects){
					...
					this.fBtnEfcts.setAttribute("aria-hidden", "true");
					...
					this.fBtnNoEfcts.setAttribute("aria-hidden", "true");
				}
			...
			this.fWaitMsg.setAttribute("aria-hidden", "true");
			...
			for (i = 0; i<vLocalSlds.length; i++){
				...
				vSldMgr = new scHPS.SldLocalMgr(vSld, vSld.fSldHdr);
				for (j = 0; j<this.fSldRules.length; j++){
					...
					} catch(e){console.warn("WARNING PresMgr.onLoad - local slide rule num. " + j + ": "+e);}
				}
			...
			if (this.fTocLnksPath){
				...
				if (vTocLnks.length>0){
					...
					for (i = 0; i < vTocLnks.length; i++){
						...
						vLnk.fPresMgr = this;
						if(vLnk.getAttribute("data-type") === "local"){
						...
						} else if(vLnk.getAttribute("data-type") === "blk"){
							...
							vLnk.fSldIdx = this.fSldUrls.length - 1;
							vLnk.onclick = scHPS.PresMgr.sOnClickTocLnk;
						}
					}
				}
	...
	addToolElement : function(pNode) {
		pNode.setAttribute("aria-live", "polite");
		if (scHPS.fDisabled) return;
	 ...
	 * @param pInstantResult : boolean - Si true précipite le résultat instannément, sans animation.
	...
	next : function(pSkip) {
		...
		if(this.fZoom.fAct && !pSkip){
			...
			if(this.fAltSlides.fSldHdr && !this.fAltSlides.fSldHdr.fIsLoaded) return false;
	...
	previous : function(pSkip) {
		...
		if(this.fZoom.fAct && !pSkip){
			...
			if (pSkip || ! vFromSld.fSldHdr.goToPrv()) {
				return this.xGotoSlide(vFromSld.fSldHdr.fSldIdx-1, (!!pSkip), false);
			}
		}
		return true;
	},
	...
	first : function() {
		this.loadSlide(this.fFirstLocalIdx, true, false);
	},
	/* --- private ------------------------------------------------------------ */
	/** PresMgr.xUpdateGui */
	xUpdateGui : function() {
	 ...
	 * @param pFromStart : boolean - Affiche le slide du début (true) ou de la fin (false).
	 * @param pInstantResult : boolean - If true no animation.
	 * @return false si echec (pas de slide d'idx pIdx, ...) */
	...
	xSetCurrSlide : function(pNewCurrSlide) {
		try{
			...
			const vNewIdx = pNewCurrSlide.fSldHdr.fSldIdx;
			const vNextIdx = vNewIdx + 1;
			const vPrevIdx = vNewIdx - 1;
			const vNextLimit = vNewIdx + 8;
			const vPrevLimit = vNewIdx - 2;
			let vIsNextOk = false;
			let vIsPrevOk = false;
			...
			for (let i=0; i<vSlds.length; i++){
				const vSld = vSlds[i];
				const vSldIdx = vSld.fSldHdr.fSldIdx;
				if(vSldIdx === vPrevIdx) {
					vIsPrevOk = true;
	 ...
	 * @param pIdx : int - slide index to build. */
	xBuildSlide : function(pIdx) {
		const vSld = scHPS.xAddEltHidden("div", this.fSldFra, this.fSlideClass);
	...
	xKeyMgr : function(pCharCode){
		...
		switch(vAction){
			...
			case "home":
				this.loadSlide(this.fFirstLocalIdx, true, false);return false;
			case "closeZoom":
				if (!this.xHideZoom()) this.xHideAltSlide(); return false;
			case "showTools":
				this.xShowTools();return false;
	...
	xShowZoom : function(pRes, pOpt){
		...
		if (!vZm.fAct){
			...
			switch(pOpt.type){
				case "sld":
					new scHPS.FadeEltTask(vZm.fFrg,0,vPresMgr,true);
					new scHPS.FadeEltTask(vZm.fSld,1,vPresMgr,true);
					vZm.fSldHdr = new scHPS.SldZoomHdr(vZm.fSld, pRes.href+(pOpt.setModeHtml?"?mode=html":""),vPresMgr,function(){new scHPS.FadeEltTask(vZm,1,vPresMgr);});
					vZm.fSldHdr.initSld();
					break;
				...
				case "fra":
					new scHPS.FadeEltTask(vZm.fFrg,0,vPresMgr,true);
					new scHPS.FadeEltTask(vZm.fSld,1,vPresMgr,true);
					vZm.fFraHdr = new scHPS.FraZmHdr(vZm.fSld, pRes.href+(pOpt.setModeHtml?"?mode=html":""),vPresMgr,function(){new scHPS.FadeEltTask(vZm,1,vPresMgr);});
					vZm.fFraHdr.init();
					break;

				default:
					new scHPS.FadeEltTask(vZm.fFrg,1,vPresMgr,true);
					new scHPS.FadeEltTask(vZm.fSld,0,vPresMgr,true);
					vDst = scHPS.importDeepNode(pRes, vZm.fFrg.ownerDocument, vZm.fFrg);
					new scHPS.FadeEltTask(vZm,1,vPresMgr);
			}
			for (let i=0; i<this.fNav.length; i++) {
				const vNav = this.fNav[i];
			...
			switch(pOpt.type){
				...
				default:
					vDst = scHPS.importDeepNode(pRes, vZm.fFrg.ownerDocument, vZm.fFrg);
					new scHPS.FadeEltTask(vZm.fSld,0,vPresMgr);
					new scHPS.FadeEltTask(vZm.fFrg,1,vPresMgr);
			}
		}
	...
	xHideZoom : function(){
		if (this.fZoom.fAct){
			...
			new scHPS.FadeEltTask(this.fZoom,0,this);
			for (let i=0; i<this.fNav.length; i++) {
				const vNav = this.fNav[i];
	...
	xShowAltSlide : function(pRes, pOpt){
		...
		if (vZm.fCurrSld) new scHPS.FadeEltTask(vZm.fCurrSld,0,vPresMgr,true);
		new scHPS.FadeEltTask(vSld,1,vPresMgr,true);
		this.fSwitchBlkTask.initTask(vZm.fSldHdr.fCurrBlk, vBlockId ? sc$(vBlockId) : vZm.fSldHdr.getFirstBlock(), "F", true);
		if (!vZm.fAct){
			vZm.fAct = true;
			new scHPS.FadeEltTask(vZm,1,vPresMgr);
			for (let i=0; i<this.fNav.length; i++) {
	...
	xHideAltSlide : function(){
		if (this.fAltSlides && this.fAltSlides.fAct){
			...
			if (vZm.fCurrSld) new scHPS.FadeEltTask(vZm.fCurrSld,0,vPresMgr,true);
			vZm.fCurrSld = null;
			new scHPS.FadeEltTask(this.fAltSlides,0,this);
	...
	xHideTools : function(){
		if (!this.fToolsHidden) {
			const vNewTime = new Date().getTime();
			if (vNewTime-this.fToolsShowTime >= 5000) {
				this.fToolsHidden = true;
				for (let i=0; i<this.fToolElts.length; i++) {
					try {
						new scHPS.FadeEltTask(this.fToolElts[i],0);
					} catch(e) {console.info("ERROR PresMgr.xHideTools : "+e);}
				}
			}
		}
	},
	...
	xShowTools : function(){
		if (this.fToolsHidden) {
			...
			for (let i=0; i<this.fToolElts.length; i++) {
				try {
					new scHPS.FadeEltTask(this.fToolElts[i],1);
				} catch(e) {console.info("ERROR PresMgr.xShowTools : "+e);}
			}
		}
		this.fToolsShowTime = new Date().getTime();
	},
	...
	fSwitchSldTask : {
		...
		execTask : function(){
			...
			}catch(e){console.error("ERROR PresMgr.fSwitchSldTask.execTask : "+e);}
		},
		...
		precipitateEndTask : function(){
			try{
				...
				if(this.fPresMgr.fCurrSld) {
					scHPS.xEndOpacityEffect(this.fPresMgr.fCurrSld, 0, this.fPresMgr.fEnableCssEffects);
					if (this.fPresMgr.fEnableCssEffects && this.fPresMgr.fCurrSld.fSldHdr.fCurrBlk) {
						try{
	...
	fSwitchBlkTask : {
		...
		execTask : function(){
			...
			}catch(e){console.error("ERROR PresMgr.fSwitchBlkTask.execTask : "+e);}
		},
		...
		precipitateEndTask : function(){
			try{
				...
				if(this.fOldBlock) {
					scHPS.xEndOpacityEffect(this.fOldBlock, 0, this.fPresMgr.fEnableCssEffects);
					if (this.fPresMgr.fEnableCssEffects && this.fOldBlock) {
						try{
	...
	fSwitchStpTask : {
		...
		initTask : function(pBlock, pTargetStep, pInstantResult){
			try{
				...
				if(pInstantResult || !this.fPresMgr.fEnableEffects) this.precipitateEndTask(pInstantResult);
				else {
		...
		execTask : function(){
			...
			try{
				...
				if(this.fCycles <= 0) {
					...
					this.fEndTime += 100;
					const vDeltaOpacityNew = Math.min(this.fNewMask.fOpacity, 4 * this.fNewMask.fOpacity / this.fCycles);
					const vDeltaOpacityOld = Math.min(this.fOldMask.fOpacity, 4 * this.fOldMask.fOpacity / this.fCycles);
					if(this.fOldMask) {
						scHPS.xSetOpacity(this.fOldMask, vDeltaOpacityOld);
					}
					scHPS.xSetOpacity(this.fNewMask, this.fNewMask.fOpacity - vDeltaOpacityNew);
					const vCurrPos = this.fBlkHdr.fBlkContent.offsetTop;
					const vNewTop = vCurrPos - (2 * (vCurrPos - this.fTargetTop) / (this.fCycles + 1));
					this.fBlkHdr.fBlkContent.style.top = vNewTop+"px";
					return true;
				}
			}catch(e){console.error("ERROR PresMgr.fSwitchStpTask.execTask : "+e);}
		},
		...
		precipitateEndTask : function(pInstant){
			try{
				...
				if (this.fPresMgr.fEnableCssEffects && this.fBlkHdr.fCurrStep >= 0 && !pInstant) this.fBlkHdr.fBlkContent.classList.add("animate");
				//On place le content
				...
				if (this.fPresMgr.fEnableCssEffects && !pInstant) {
...
scHPS.PresMgr.sShowWait = function(pMgr){
	if (pMgr.fWaitMsg) {
		pMgr.fWaitMsg.style.visibility = "";
		pMgr.fWaitMsg.removeAttribute("aria-hidden");
	}
}
/** PresMgr.sHideWait. */
scHPS.PresMgr.sHideWait = function(pMgr){
	if (pMgr.fWaitMsg) {
		pMgr.fWaitMsg.style.visibility = "hidden";
		pMgr.fWaitMsg.setAttribute("aria-hidden", "true");
	}
}
 ...
 * @param pRootPath : string - slide root path.
 * @param pWin : window - current window.
 * @param pSldHdr : object - slide handler (optional). */
...
scHPS.SldMgr.prototype = {
	...
	onLoad : function(){
		try{
			...
			if (vPresMgr.fOwnerWindow !== this.fWin){
				this.fWin.addEventListener("mousemove", function(){scHPS.sMouseMgr(vPresMgr)},true);
				this.fWin.addEventListener("mousedown", function(){scHPS.sMouseMgr(vPresMgr)},true);
				this.fWin.addEventListener("keyup", function(pEvt){scHPS.sOnKeyUp(pEvt,vPresMgr)},false);
				this.fWin.addEventListener("keydown", function(pEvt){scHPS.sOnKeyDown(pEvt,vPresMgr)},false);
				if (scHPS.fScreenTouch){
	...
	xFixRatioNormalScreen : function(){
		...
		const vBaseFontSize = Math.round(this.fRatioNormalScreen * this.fDefaultFontSize);
...
scHPS.SldLocalMgr = function(pRootNode,pSldHdr){
	try{
		...
		this.fBlksPath = scHPS.fBlksPath;
		this.fDefaultAnimStep = scHPS.fDefaultAnimStep;
	}catch(e){console.error("ERROR SldLocalMgr : "+e);}
}
...
scHPS.SldHdr.prototype = {
	...
	initSld : function(){
		...
		this.fCurrBlk = null;
		this.fFraNode.onload = scHPS.SldHdr.sOnloadSlide;
	...
	/** SldHdr.goToBlk : Navigue à l'intérieur du slide pour ateindre un block. */
			...
			const iHideAll = function (pHdr) {
				...
				if (vSubs) {
					for (let i = 0; i < vSubs.length; i++) {
						scHPS.xEndOpacityEffect(vSubs[i], 0);
						if (vSubs[i].fBlkHdr && vSubs[i].fBlkHdr.fSubBlocks) iHideAll(vSubs[i].fBlkHdr);
					}
				}
			};
			iHideAll(this);
			this.fPresMgr.fSwitchBlkTask.initTask(null, pBlk, "F", false);
			vHdr = pBlk.fBlkHdr
...
scHPS.SldHdr.sOnloadSlide = function(){
	let vSldDoc;
	try{
		try{vSldDoc = this.contentWindow.document;}catch(e){}
		const vSldHdr = this.fSldHdr;
		const vPresMgr = vSldHdr.getPresMgr();
		if (typeof vSldDoc == "undefined") {
			const vAsw = confirm(scHPS.xGetStr(23));
			if (vAsw) scHPS.xSwitchToHtmlMode(true);
			else throw "Unable do access slide document."
		}
		scHPS.swichToSsStyles(vSldDoc);
		vSldHdr.fFraNode.title = vSldDoc.title;

		//Load du SldMgr dans l'iframe.
		const vSubSldMgr = vSldHdr.getSldMgr();
		vSubSldMgr.onLoad();
		...
		vSldHdr.postOnLoad();
	} catch(e){console.error("ERROR SldHdr.sOnloadSlide : "+e);}
}
...
/** SldLocalHdr.showAltSlide */
scHPS.SldLocalHdr.prototype.showAltSlide = function(pRes, pOpts){
...
scHPS.SldAltHdr.prototype = new scHPS.SldLocalHdr();
...
scHPS.FraZmHdr.prototype.init = function(){
	...
	this.fCurrBlk = null;
	this.fFraNode.onload = scHPS.FraZmHdr.sOnLoad;
...
scHPS.FraZmHdr.sOnLoad = function() {
	...
	} catch(e){console.error("ERROR FraZmHdr.sOnLoad : "+e);}
}
...
//On précipite les animations en cours sur le steps.
	...
	xPlanSteps : function(){
		...
		let vBgColor = scHPS.xReadStyle(this.fBlkContent, "backgroundColor") || "white";
		...
		for(i = this.fMasks.length; i < this.fSteps.length; i++) {
			...
			vMask = scHPS.xAddElt("div", this.fBlkContent, "ssMask "+(vStep.fForcedMask ? "ssForced" : ""));
...
scHPS.AnimBlk = function(pElt, pOpt){
	...
	if (this.fOpt.counter){
		const vCounter = scHPS.xAddElt("span", pElt.fCtrl, this.fSsClassPrefix + "Counter");
...
scHPS.AnimBlk.sShowCtrl = function(pAnim){
	const vPresMgr = pAnim.fAnimBlk.fPresMgr;
...
scHPS.SizeBlkObj = function(pElt, pOpt){
	...
	pElt.fObj = vObj;
	const vVideoElt = scPaLib.findNode("cde:video");
	if (vVideoElt){
...
scHPS.SizeBlkSvg.prototype.onResizedAnc = function(pOwnerNode, pEvent){
	if(pEvent.phase===1) {
		try{
			...
			if (pOwnerNode.fResFra){
				pOwnerNode.fResFra.style.width=vSvg.fWidth+"px";
				pOwnerNode.fResFra.style.height=vSvg.fHeight+"px";
			}
			vSvg.style.width=vSvg.fWidth+"px";
			vSvg.style.height=vSvg.fHeight+"px";
...
scHPS.SizeBlkSvg.sOnClickZoom = function(pEvent) {
	...
	const vResizer = {
		...
		onResizedAnc: function (pOwnerNode, pEvent) {
			...
			vZmItm.style.width = vZmItm.fWidth + "px";
			vZmItm.style.height = vZmItm.fHeight + "px";
			vZmItm.style.marginTop = Math.max(0, (vMaxH - vZmItm.fHeight) / 2) + "px";
		}
	};
	scSiLib.addRule(vZmItm, vResizer);
	vResizer.onResizedAnc(vZmItm);
